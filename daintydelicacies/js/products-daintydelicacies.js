/**
 * 🧁 Dainty Delicacies - Centralized Product Data & Utilities (ULTRA PERFORMANCE EDITION)
 * 📁 Path: /daintydelicacies/js/products-daintydelicacies.js
 * ✅ FIXED: Removed all syntax errors, fixed template literals, and exposed window.DD_PRODUCTS.
 */
(function () {
    'use strict';
    ...
    const CONFIG = {
        SHEETS_API_URL: "https://script.google.com/macros/s/AKfycbwl4tSfxGu5zKR1L-TzBgsh-7eG3gNU4nomcNEK8NBM5-uAtCBtHEAvjYwHLy_ADQi9/exec",
        basePath: "",
        ...
        CACHE_KEY: "dd_products_cache",
        CART_KEY: "dd_cart",
        CACHE_TTL: 10 * 60 * 1000, // 10 minutes
        WHATSAPP_NUMBER: "27840967330",
        ...
        try {
            if (!forceRefresh) {
                ...
                if (cached && cached.length > 0) {
                    console.log('⚡ Loaded Dainty Delicacies products from cache (instant)');
    ...
    async function backgroundRefresh(knownHash) {
        ...
        try {
            ...
            if (lastRawSnapshot === null) lastRawSnapshot = JSON.stringify(window.DD_INLINE_PRODUCTS || window.DD_PRODUCTS || []);
    ...
    function processProducts(rawProducts) {
        ...
        // ✅ CRITICAL FIX: Expose to window.DD_PRODUCTS so index.html can render them!
        window.DD_PRODUCTS = PRODUCTS;
        window.DAINTY_DELICACIES_PRODUCTS = PRODUCTS;
        window.DAINTY_DATA = PRODUCTS;
        return PRODUCTS;
    }
    ...
    const Cart = {
        ...
        async add(productId, quantity = 1) {
            ...
            this.save();
            this.updateUI();
            this.dispatchEvent('dd:cart:updated', { cart: this.items, addedProduct: product });
            return true;
        },
        async remove(productId) {
            ...
            this.save();
            this.updateUI();
            this.dispatchEvent('dd:cart:updated', { cart: this.items });
            return true;
        },
    ...
    window.DaintyProducts = {
        ...
        renderCard: (p) => {
            ...
            return `<article class="product-card" data-id="${p.id}" data-category="${p.categorySlug}" data-price="${p.price}" data-name="${p.name}" data-description="${p.description}" data-image="${p.image}" onclick="openProductModal('${p.id}')" role="button" tabindex="0" style="cursor:pointer;">
                ...
                <div class="product-info">
                    ...
                    <button class="add-to-cart-btn" onclick="event.stopPropagation(); DaintyProducts.addToCart('${p.id}'); return false;">
                        ${btnText}
                    </button>
                </div>
            </article>`;
        ...
        openModal: (productId) => {
            ...
            else document.dispatchEvent(new CustomEvent('dd:modal:open', { detail: { productId } }));
        },
        getStatus: () => ({
    ...
    (async function init() {
        ...
        // ✅ FIXED: Check for both DD_INLINE_PRODUCTS and DD_PRODUCTS
        const inlineData = window.DD_INLINE_PRODUCTS || window.DD_PRODUCTS;
        ...
        if (hasInlineData) {
            ...
            try {
                document.dispatchEvent(new CustomEvent('dd:products:loaded', { detail: { products: PRODUCTS } }));
            } catch (err) {}
            setTimeout(() => backgroundRefresh(window.DD_INLINE_PRODUCTS_HASH), 1500);
        } else {
            await fetchProducts();
            try {
                document.dispatchEvent(new CustomEvent('dd:products:loaded', { detail: { products: PRODUCTS } }));
        ...
        console.group('🧁 Dainty Delicacies Products Initialized');
    ...
    function generateProductSchema() {
        ...
        const productList = {
            ...
            "@type": "ItemList",
            "name": "Dainty Delicacies Product Catalog",
            "description": "Artisanal mini cakes, bundts, logs, and promo boxes from Dainty Delicacies.",
            ...
            "itemListElement": PRODUCTS.map((product, index) => ({
                ...
                "item": {
                    ...
                    "image": product.image.startsWith('http') ? product.image : `https://daintydelicacies.co.za/${product.image}`,
                    "sku": product.id,
                    "brand": { "@type": "Brand", "name": "Dainty Delicacies" },
                    "offers": {
                        "@type": "Offer",
                        "priceCurrency": "ZAR",
                        ...
                        "seller": { "@type": "Organization", "name": "Dainty Delicacies" }
                    }
                }
            }))
        };

        let schemaEl = document.getElementById('ddProductSchema');
        if (!schemaEl) {
            schemaEl = document.createElement('script');
            schemaEl.type = 'application/ld+json';
            schemaEl.id = 'ddProductSchema';
            document.head.appendChild(schemaEl);
        }
        schemaEl.textContent = JSON.stringify(productList);
    }

    document.addEventListener('dd:products:loaded', () => setTimeout(generateProductSchema, 500));
